...
figure(1)
mesh(m)
title('Gabor filter for d4d29'), xlabel('x'), ylabel('y'), zlabel('m(x,y)')

%--------------------------- Smoothing filter -----------------------------
sigma_s = 40;    % for smoothing filter 
i_p = zeros(M,N);
m_p = zeros(M,N);
...
figure(3)
mesh(m_prime)
title('Smoothing filter for d4d29'), xlabel('x'), ylabel('y'), zlabel('m^p(x,y)')
